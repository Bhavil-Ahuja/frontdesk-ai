...
Regression tests for recent feature additions:
  ...
  - Vapi integration routes (connect / sync / disconnect / status)
  - vapi_service.register_assistant honours tenant_ctx.voice_config
...
def _make_mock_tenant(**overrides):
    ...
    t.vapi_api_key = overrides.get("vapi_api_key", "vapi_test_key")
    t.vapi_assistant_id = overrides.get("vapi_assistant_id", "")
    t.vapi_phone_number_id = overrides.get("vapi_phone_number_id", "")
    t.vapi_webhook_secret = overrides.get("vapi_webhook_secret", "")
...
def legacy_test_vapi_register_uses_voice_config():
    ...
    ctx = _make_ctx(
        vapi_api_key="key_test",
        vapi_assistant_id="",  # force POST (create)
        voice_config={"provider": "11labs", "voiceId": "voice_custom_xyz"},
    )
    ...
    class _MockResp:
        ...
        def json(self): return {"id": "asst_new_123"}
    ...
    async def _run():
        with patch.object(vapi_service.http, "post", side_effect=_mock_post):
            return await vapi_service.register_assistant(tenant_ctx=ctx)

    result = asyncio.run(_run())
    _assert(result == "asst_new_123", "returned new assistant id")
    payload = captured.get("json", {})
    voice = payload.get("voice", {})
    _assert(voice.get("voiceId") == "voice_custom_xyz",
            f"payload uses tenant's voiceId (got {voice.get('voiceId')})")
    _assert(voice.get("provider") == "11labs", "payload uses tenant's voice provider")
    # Custom-LLM webhook wiring
    model = payload.get("model", {})
    _assert(model.get("provider") == "custom-llm", "model.provider=custom-llm")
...
def legacy_test_vapi_register_falls_back_when_voice_config_missing():
    ...
    class _MockResp:
        def raise_for_status(self): pass
        def json(self): return {"id": "asst_fb"}
    ...
    asyncio.run(_run())
    voice = captured.get("json", {}).get("voice", {})
    _assert(voice.get("voiceId") == "21m00Tcm4TlvDq8ikWAM",
            "falls back to Rachel voiceId when voice_config empty")
    _assert(voice.get("provider") == "11labs", "falls back to 11labs provider")
...
def legacy_test_vapi_integration_routes_registered():
    ...
    expected = {
        "/api/integrations/vapi/connect",
        "/api/integrations/vapi/sync",
        "/api/integrations/vapi/disconnect",
        "/api/integrations/vapi/status",
    }
    for p in expected:
...
def legacy_test_vapi_connect_request_schema():
    ...
    # Valid: api_key only
    req = VapiConnectRequest(api_key="sk_vapi_abcdefghij")
    _assert(req.api_key == "sk_vapi_abcdefghij", "api_key accepted")
    _assert(req.phone_number_id is None, "phone_number_id optional")

    # Valid with phone_number_id
    req2 = VapiConnectRequest(api_key="sk_vapi_abcdefghij", phone_number_id="pn_123")
    _assert(req2.phone_number_id == "pn_123", "phone_number_id captured")

    # Too-short api_key rejected
    try:
        VapiConnectRequest(api_key="short")
        _assert(False, "should reject too-short api_key")
...
def legacy_test_vapi_router_mounted_on_app():
    ...
    _assert("/api/integrations/vapi/connect" in paths,
            "/api/integrations/vapi/connect mounted on FastAPI app")
    _assert("/api/integrations/vapi/status" in paths,
            "/api/integrations/vapi/status mounted on FastAPI app")
