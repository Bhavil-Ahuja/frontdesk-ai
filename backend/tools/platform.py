...
class PlatformToolProvider:
    ...
    async def _get_available_slots(
        ...
        _subject_not_offered = provider_slots_result.get("subject_filter")
        if _subject_not_offered and not provider_slots_result.get("slots"):
            logger.info(
                "[Call %s] get_available_slots: no faculty for subject=%r",
                call_id, _subject_not_offered,
            )
            return {
                "ok": False,
                "error": "subject_not_offered",
                "date": date,
                "subject": _subject_not_offered,
                "summary_for_assistant": (
                    f"No faculty at this institute currently teach {_subject_not_offered!r}. "
                    f"Do NOT say 'no slots available' or 'fully booked' or 'closed'. "
                    f"Tell the caller: 'We don\u2019t currently offer {_subject_not_offered} coaching.' "
                    f"Ask if they\u2019d like to book a demo for one of our other subjects, "
                    f"or offer to have a counselor follow up."
                ),
                "available_slots": [],
                "count": 0,
                "no_subject_coverage": True,
            }
    ...
    async def _book_appointment(
        ...
        if result:
            ...
            coaching_extra = {}
            for field in ("student_name", "grade", "board", "target_exam", "medium"):
                val = args.get(field, "")
            ...
            if not tenant_ctx:
                try:
                    ...
                    await caller_service.record_session(
                        ...
                        student_name=args.get("caller_name", ""),
                        student_email=email,
                        dob=args.get("dob", ""),
                        tenant_id=None,
                        provider_id=provider_id,
    ...
    async def _update_caller_info(
        ...
        async with get_async_session() as db_session:
            ...
            if not updated_fields:
                return {
                    "ok": False,
                    "summary_for_assistant": "No fields to update were provided. Specify at least one of: name, dob, email, notes, student_name, grade, board, target_exam, candidate_type, attempt_number, mode_preference, medium.",
                }
